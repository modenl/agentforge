<script>
  ...
  function handleExternalMessage(messageText) {
    console.log('📨 收到外部消息:', messageText);
    sendMessageToCoreAgent(messageText);
  }

  // 处理输入辅助卡片按钮点击
  function handleInputAssistCardAction(event) {
    console.log('🎯 InputAssist Card Action:', event.detail);
    ...
    if (hasPasswordInput && actionData && actionData.data) {
      ...
      if (passwordValue) {
        // 标记这是密码输入，这样可以被正确遮罩
        console.log('🔐 Detected password input from Input.Text style="password":', '••••••');
